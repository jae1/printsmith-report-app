    ...
    def process_paid_rows(rows):
        ...
        for inv, d in aggregated.items():
            ...
            if not actual_payment_today and d.get("is_job_posted", 0) > 0:
                ...
                cur_detail.execute("SELECT SUM(totalpay) FROM tapeinvoicepayrecord WHERE invoicenumber = %s AND CAST(\"date\" AS DATE) = %s AND isdeleted = false AND method != 'Deposit'", (inv, target_date))
                tip_today = float(cur_detail.fetchone()['sum'] or 0)
                cur_detail.execute("SELECT SUM(amountpaid) FROM tapesalerecord WHERE invoicenumber = %s AND CAST(\"date\" AS DATE) = %s AND isdeleted = false AND paymode != 'Charge' AND paymode != 'Deposit'", (inv, target_date))
